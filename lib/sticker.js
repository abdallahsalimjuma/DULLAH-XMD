 ...
 */
const _xteamCalls = []
const _XTEAM_LIMIT = 10
const _XTEAM_WINDOW = 60000

async function sticker3(img, url, packname, author) {
  const now = Date.now()
  _xteamCalls.splice(0, _xteamCalls.length, ..._xteamCalls.filter(t => now - t < _XTEAM_WINDOW))
  if (_xteamCalls.length >= _XTEAM_LIMIT) throw new Error('Rate limit exceeded for sticker API')
  _xteamCalls.push(now)
...
function sticker6(img, url) {
  return new Promise(async (resolve, reject) => {
    ...
    const out = path.join(__dirname, '../tmp', `${+ new Date()}.webp`)
